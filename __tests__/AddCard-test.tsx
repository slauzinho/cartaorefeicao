...
import { render, fireEvent, flushMicrotasksQueue, waitForElement } from 'react-native-testing-library';
...
function renderWithRedux(ui, { initialState, store = createStore(reducer, initialState) }: any = {}) {
    return {
...
describe('AddCard', () => {
    ...
    it('allows me to submit my santander card details', async () => {
        ...
        const { getByType, getByText } = renderWithRedux(<AddCard {...(props as any)} />);
    ...
    it('allows me to choose different colours', async () => {
        ...
        const { getByTestId, getByType } = renderWithRedux(<AddCard {...(props as any)} />);
    ...
    it('flips card when user focus inputs', () => {
        const props = createTestProps();
        const { getByText, getByType } = renderWithRedux(<AddCard {...(props as any)} />);
    ...
    it('opens the additional info modal', () => {
        const props = createTestProps();
        const { getByType, getByTestId } = renderWithRedux(<AddCard {...(props as any)} />);

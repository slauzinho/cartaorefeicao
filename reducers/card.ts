...
export function card(state: CardState = { cards: [], loading: true }, action: CardActionTypes) {
    switch (action.type) {
        case ADD_CARD_REQUEST:
            return { ...state, loading: true };
        case ADD_CARD_SUCCESS:
            const oldCardsFiltered = state.cards.filter(card => card.cardNumber !== action.card.cardName);
            return {
                ...state,
                loading: false,
        ...
        case REMOVE_CARD_SUCCESS:
            return {
                ...state,
                loading: false,
                cards: state.cards.filter(card => card.cardNumber !== action.cardNumber),
            };

...
import { fetchCardsSuccess, fetchSiteSuccess, fetchSiteFailure, removeCardSuccess, addCardSuccess } from '../actions';
import api from '../api';
import { FetchSiteRequest, RemoveCardRequest, AddCardRequest } from '../actions/types';
...
export function* addCard(action: AddCardRequest) {
    yield call(AsyncStorage.setItem, action.card.cardNumber, JSON.stringify(action.card));
